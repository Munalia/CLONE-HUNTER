...
class GoogleDriveHelper:
    ...
    def clone(self, link, status, ignoreList=[]):
        ...
        if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
            ...
            msg += f'📁 File Name: <a href="{self.__G_DRIVE_DIR_BASE_DOWNLOAD_URL.format(dir_id)}">{meta.get("name")}</a>' \
                   f'\n💽 Size: ({get_readable_file_size(self.transferred_size)})'
            if INDEX_URL:
                url = requests.utils.requote_uri(f'{INDEX_URL}/{meta.get("name")}/')
                msg += f'\n<a href="{url}">🔗 Index URL</a>'
        else:
            try:
                ...
                LOGGER.error(err)
                return err
            msg += f'📁 File Name: <a href="{self.__G_DRIVE_BASE_DOWNLOAD_URL.format(file.get("id"))}">{file.get("name")}</a>'
            try:
